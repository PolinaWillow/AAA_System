...
const iv256 = require( "./Constants.js")
const iv512 = require( "./Constants.js")
const N_0 = require( "./Constants.js")
const PI = require( "./Constants.js")
const TAU = require( "./Constants.js")
...
class Strybog{
    ...
    TranslateMessageToByte(message){
        const utf8Encode = new TextEncoder();
        const byteArr = utf8Encode.encode(message);
        return byteArr
    }

    TranslateMessageToString(message){       
        let string = new TextDecoder().decode(message);
        return string;
    }

    GetHash(message){
        let message_byte = this.TranslateMessageToByte(message)
        if((this._bit!=512)&&(this._bit!=265)) return -1
        let paddedMsg = new Uint8Array(BLOCK_SIZE);
        let h = new Uint8Array(BLOCK_SIZE);
        let len = message_byte.length;
        ...
        while (len >= BLOCK_SIZE) {
            ...
            let pos = message_byte.length - inc * BLOCK_SIZE;
            tmpMsg = message_byte.slice(pos, pos + BLOCK_SIZE);
        ...
        let msg = new Uint8Array(message_byte.length - inc*64);
        msg = message_byte.slice(0, message_byte.length - inc*64);
        ...
        if (this._bit == 512) return this.TranslateMessageToString(h);
        else return this.TranslateMessageToString(h.slice(0, 32));
    }

}

module.exports = new Strybog();